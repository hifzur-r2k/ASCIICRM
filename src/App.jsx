...
import { 
  ...
  LogOut, Lock, Plus, X, Layers, IndianRupee
} from 'lucide-react';
...
export default function App() {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      for (let r = 0; r < rows.length; r++) {
        ...
        } else if ((col0.includes("MASON") || col1.includes("MASON")) && !col0.includes("HALF") && !col0.includes("HALF")) {
          activeSection = "Mason";
        }
  ...
  // --- SINGLE-SITE ANALYTICS LOGIC (WITH FULL OT & SPEND) ---
  const getGlobalAnalytics = () => {
    ...
    savedSheets.forEach(sheet => {
      ...
      if (siteMatch) {
        ...
        totals.totalSpend += (siteMatch.totalBaseCost || 0) + (siteMatch.totalOTCost || 0);
      }
    });
  ...
  // --- MULTI-SITE MANUAL INPUT LOGIC (WITH FULL OT & SPEND) ---
  ...
  const getMultiSiteAnalytics = () => {
    ...
    compareSitesList.forEach(siteCode => {
      ...
      savedSheets.forEach(sheet => {
        const match = (sheet.siteData || []).find(s => s.site.toLowerCase() === siteCode.toLowerCase());
        if (match) {
          sTotals.occurrences++;
          sTotals.masonReg += match.masonReg || 0;
          sTotals.masonOT += match.masonOT || 0;
          sTotals.halfMasonReg += match.halfMasonReg || 0;
          sTotals.halfMasonOT += match.halfMasonOT || 0;
          sTotals.helperReg += match.helperReg || 0;
          sTotals.helperOT += match.helperOT || 0;
          sTotals.siteSpend += (match.totalBaseCost || 0) + (match.totalOTCost || 0);
        }
      });
      ...
      totals.masonReg += sTotals.masonReg;
      totals.masonOT += sTotals.masonOT;
      totals.halfMasonReg += sTotals.halfMasonReg;
      totals.halfMasonOT += sTotals.halfMasonOT;
      totals.helperReg += sTotals.helperReg;
      totals.helperOT += sTotals.helperOT;
      totals.totalDays += totalDays;
      totals.totalOT += totalOT;
  ...
  const totals = filteredData.reduce((acc, row) => {
    ...
    acc.totalBaseCost += row.totalBaseCost || 0;
    acc.totalOTCost += row.totalOTCost || 0;
    return acc;
  ...
  const exportToExcel = () => {
    const formattedData = filteredData.map(row => {
      ...
      return {
        ...
        "Base Cost (Rs)": row.totalBaseCost || 0,
        "OT Cost (Rs)": row.totalOTCost || 0,
    ...
    totalRow["Base Cost (Rs)"] = totals.totalBaseCost;
    totalRow["OT Cost (Rs)"] = totals.totalOTCost;
  ...
  const exportToPDF = () => {
    ...
    doc.text("CRM_FIX FINANCIAL EXPORT", 14, 15);
    doc.setFontSize(10);
      ...
      {activeData.length === 0 ? (
        <div className="max-w-6xl mx-auto pt-10 md:pt-16 px-4 pb-12">
          ...
          <div className="text-center mb-10 space-y-3">
            ...
            <p className="text-sm md:text-lg font-medium text-gray-500 uppercase tracking-widest">Financial & Attendance Engine</p>
          </div>
          ...
          {dashboardTab === 'upload' && (
            <div onClick={() => fileInputRef.current?.click()} className="group w-full max-w-2xl mx-auto cursor-pointer bg-white rounded-3xl border-2 border-dashed border-gray-300 hover:border-blue-500 hover:bg-blue-50/50 transition-all duration-300 p-8 md:p-20 shadow-sm hover:shadow-xl text-center">
              <div className="flex flex-col items-center justify-center space-y-6">
                ...
                <div><p className="text-xl md:text-2xl font-bold text-gray-800">Upload Financial Sheet</p><p className="text-sm md:text-base text-gray-500 mt-2">Parse and visualize site labor costs instantly.</p></div>
          ...
          {dashboardTab === 'analytics' && (
            <div className="max-w-5xl mx-auto space-y-6">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-200 text-center">
                <h2 className="text-lg md:text-xl font-black text-gray-900 mb-4">Master Site Analytics</h2>
                
                <div className="flex justify-center mb-6">
                  <div className="bg-gray-100 p-1.5 rounded-xl inline-flex shadow-inner">
                    <button onClick={() => setAnalyticsMode('single')} className={`px-5 py-2 rounded-lg font-bold text-sm transition-all flex items-center gap-2 ${analyticsMode === 'single' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}><Search className="w-4 h-4"/> Single Site Lookup</button>
                    <button onClick={() => setAnalyticsMode('compare')} className={`px-5 py-2 rounded-lg font-bold text-sm transition-all flex items-center gap-2 ${analyticsMode === 'compare' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}><Layers className="w-4 h-4"/> Multi-Site Compare</button>
                  </div>
                </div>
                  ...
                  <div className="w-full max-w-2xl mx-auto">
                    <form onSubmit={handleAddCompareSite} className="flex flex-col sm:flex-row items-center gap-3">
                      <div className="relative flex-1 w-full">
                        <Plus className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
                        <input type="text" placeholder="Type site code (e.g. S01) & press Enter..." value={compareSiteInput} onChange={(e) => setCompareSiteInput(e.target.value)} className="w-full pl-12 pr-4 py-3.5 md:py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm md:text-base font-bold focus:outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-inner"/>
                      </div>
                      <button type="submit" className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-6 py-3.5 md:py-4 rounded-2xl font-black flex items-center justify-center gap-2 transition-colors shadow-md">Add</button>
                    </form>
                    {compareSitesList.length > 0 && (
                      <div className="flex flex-wrap gap-2 justify-center mt-5">
                        {compareSitesList.map(site => (
                          <div key={site} className="bg-blue-50 border border-blue-200 text-blue-800 px-4 py-2 rounded-xl text-sm font-black flex items-center gap-2 shadow-sm">
                            {site}
                ...
                <>
                  {analyticsData.results.length === 0 ? (
                    ...
                    <div className="space-y-6">
                            ...
                            <div key={idx} className="p-4 hover:bg-gray-50 transition-colors flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                              <div className="max-w-full md:max-w-[200px]">
                                ...
                                <p className="text-xs font-semibold text-gray-400 flex items-center gap-1 mt-0.5">
                                  <Clock className="w-3 h-3" /> {new Date(record.date).toLocaleDateString()} at {new Date(record.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                </p>
                              </div>
                              <div className="flex gap-2 md:gap-4 flex-wrap w-full md:w-auto justify-between md:justify-end items-center">
                                <div className="text-center bg-white p-2 rounded-lg border border-gray-100 flex-1 md:flex-none min-w-[75px]">
                                ...
                                <div className="text-center bg-emerald-50 p-2 rounded-lg border border-emerald-100 flex-1 md:flex-none min-w-[95px]">
                                  <p className="text-[9px] md:text-[10px] font-bold text-emerald-600 uppercase">Sheet Cost</p>
                      ...
                      {/* COMPLETE GLOBAL TOTALS GRID WITH OT */}
                      <div className="bg-gray-900 p-6 md:p-8 rounded-3xl shadow-2xl border border-gray-800">
                        <h3 className="text-white font-black text-center text-lg md:text-xl mb-6 tracking-wider flex items-center justify-center gap-2">
                          <Database className="w-5 h-5 text-blue-400" /> GLOBAL TOTALS FOR "{globalSearch.toUpperCase()}"
                        </h3>
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 text-xs">
                          <div className="bg-gray-800/80 p-3.5 rounded-2xl">
                            <p className="text-gray-400 font-bold mb-1 uppercase text-[10px] tracking-wider">Mason Days</p>
                            <p className="text-2xl md:text-3xl font-black text-white">{analyticsData.totals.masonReg}</p>
                          </div>
                          <div className="bg-gray-800/80 p-3.5 rounded-2xl">
                            <p className="text-gray-400 font-bold mb-1 uppercase text-[10px] tracking-wider">Mason Extra</p>
                            <p className="text-2xl md:text-3xl font-black text-white">{analyticsData.totals.masonOT} <span className="text-xs font-medium text-gray-500">hrs</span></p>
                          </div>
                          <div className="bg-gray-800/80 p-3.5 rounded-2xl">
                            <p className="text-purple-400 font-bold mb-1 uppercase text-[10px] tracking-wider">HM Days</p>
                            <p className="text-2xl md:text-3xl font-black text-purple-200">{analyticsData.totals.halfMasonReg}</p>
                          </div>
                          <div className="bg-gray-800/80 p-3.5 rounded-2xl">
                            <p className="text-purple-400 font-bold mb-1 uppercase text-[10px] tracking-wider">HM Extra</p>
                            <p className="text-2xl md:text-3xl font-black text-purple-200">{analyticsData.totals.halfMasonOT} <span className="text-xs font-medium text-gray-500">hrs</span></p>
                          </div>
                          <div className="bg-gray-800/80 p-3.5 rounded-2xl">
                            <p className="text-orange-400 font-bold mb-1 uppercase text-[10px] tracking-wider">Helper Days</p>
                            <p className="text-2xl md:text-3xl font-black text-white">{analyticsData.totals.helperReg}</p>
                          </div>
                          <div className="bg-gray-800/80 p-3.5 rounded-2xl">
                            <p className="text-orange-400 font-bold mb-1 uppercase text-[10px] tracking-wider">Helper Extra</p>
                            <p className="text-2xl md:text-3xl font-black text-white">{analyticsData.totals.helperOT} <span className="text-xs font-medium text-gray-500">hrs</span></p>
                          </div>
                          <div className="bg-blue-600/90 p-3.5 rounded-2xl border border-blue-500">
                            <p className="text-blue-100 font-bold mb-1 uppercase text-[10px] tracking-wider">Total Man-Days</p>
                            <p className="text-2xl md:text-3xl font-black text-white">{analyticsData.totals.masonReg + analyticsData.totals.halfMasonReg + analyticsData.totals.helperReg}</p>
                          </div>
                          <div className="bg-emerald-600 p-3.5 rounded-2xl border border-emerald-500">
                            <p className="text-emerald-100 font-bold mb-1 uppercase text-[10px] tracking-wider">Total Spend</p>
                            <p className="text-2xl md:text-3xl font-black text-white">{formatCurrency(analyticsData.totals.totalSpend)}</p>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              {/* MULTI-SITE COMPARE RENDER (WITH FULL OT COLUMNS) */}
